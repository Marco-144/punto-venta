...
const INVENTARIO_STORAGE_KEY = "punto_venta_inventario_v1";
...
function cargarInventarioPersistido() {
    ...
    if (!raw) {
        return INVENTARIO_BASE.map((articulo) => ({ ...articulo, sucursales: { ...articulo.sucursales } }));
    }
...
function guardarInventarioPersistido() {
    localStorage.setItem(INVENTARIO_STORAGE_KEY, JSON.stringify(inventarioDB));
}

let inventarioDB = cargarInventarioPersistido();
...
function iniciarVistaInventario() {
    ...
    function renderizarTabla() {
        ...
        tablaInventario.innerHTML = articulosFiltrados
            .map(
                (articulo) => `
                    <tr data-id="${articulo.id}" class="border-b border-gray-100 dark:border-gray-700">
                        <td class="px-6 py-3 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">${articulo.folio}</td>
                        <td class="px-6 py-3 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">${articulo.codigo}</td>
                        <td class="px-6 py-3 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">${articulo.articulo}</td>
                        <td class="px-6 py-3 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">${formatoMoneda(articulo.precio)}</td>
                        <td class="px-6 py-3 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">${articulo.existencias}</td>
                        <td class="px-6 py-3">
                            <div class="flex items-center gap-2">
                                <button type="button" data-action="eliminar" data-id="${articulo.id}" class="inline-flex items-center justify-center w-9 h-9 rounded-lg border border-blue-200 bg-blue-50 text-blue-700 hover:bg-blue-600 hover:text-white active:bg-blue-700 active:scale-95 transition-all duration-150 focus:outline-none focus:ring-2 focus:ring-blue-300 dark:bg-blue-900/40 dark:border-blue-700 dark:text-blue-300"><svg class="w-6 h-6" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
                                    <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 7h14m-9 3v8m4-8v8M10 3h4a1 1 0 0 1 1 1v3H9V4a1 1 0 0 1 1-1ZM6 7h12v13a1 1 0 0 1-1 1H7a1 1 0 0 1-1-1V7Z"/>
                                    </svg>
                                </button>
                                <button type="button" data-action="editar" data-id="${articulo.id}" class="inline-flex items-center justify-center w-9 h-9 rounded-lg border border-red-200 bg-red-50 text-red-700 hover:bg-red-600 hover:text-white active:bg-red-700 active:scale-95 transition-all duration-150 focus:outline-none focus:ring-2 focus:ring-red-300 dark:bg-red-900/40 dark:border-red-700 dark:text-red-300"><svg class="w-6 h-6 text-gray-800 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
